# WARNING! This program will create practice log files on your machine
import os
import sys
import time
import psutil
...
def startup():
    ...
    print("********************")
    print("WARNING! This program will create 10 practice log files on your machine!")
    ...
    if choice.lower() == "y":
        ...
        while attempts >= 0:        
            ...
            if partition.lower() == "q":
                sys.exit()
            elif len(partition) != 1 or not partition.isalpha():
...
def path_verification():
    tries = 4
    while tries >= 0:
        path = input("Where would you like the test logs created? ")
        if path.lower() == "q":
            sys.exit()
    ...
    while i <= 10:
        ...
        while z <= 100:
            ...
            for option in (hostnames, systems, authentication, networking, files, jobs, security, levels, oses, messages):
                ...
                listy.append(pick)
            with open(rf"{location}\\system_log_{datetime.now().strftime('%Y-%m-%d')}_{file_id}_{i}.txt", "a") as f:
                f.write(f"{datetime.now()} {listy[0]} {listy[1]}[{x}]: {listy[7]}: {listy[9]} (event_id={y})\n")         
            z += 1
        i += 1
    
    print("***\nOperation Successful!\nFolder created at path: {}".format(location))
    print("Generated 10 practice log files (100 lines of random data).\n***")
    
    while True:
        question = input("Would you like to create 10 more practice files? (y/n): ").lower()
        if question == "y":
            i = 1
            while i <= 10:
                z = 1
            ...
            print("Thank you for choosing Log Creator v1.0.\nThis application will close in 3 seconds.")
